...
<body>
    ...
    <div class="black-border">
        ...
        <div class="header-a">
            <p>แบบขออนุมัติเดินทางไปปฏิบัติงาน5555</p>
        </div>
            ...
            <tbody>
                ...
                foreach ($workassignment as $show) {
                    ...
                    if ($matches) {
                        $province = $matches[1];
                        $provinces[] = $province;
                        if ($rowIndex > 0 && $province !== $provinces[$rowIndex - 1]) {
                ...
                foreach ($workassignment as $index => $show) {
                    $locationChunks = splitTextIntoLines($show->location, 70);
                    ...
                    for ($i = 0; $i < $maxChunks; $i++) {
                        ...
                        if ($isSameProvince) {
                            ...
                            if ($i == 0) {
                                $currentProvince = $provinces[$index];
                                $nextProvince = $provinces[($index + 1) % count($provinces)];
                                if ($currentProvince !== $nextProvince) {
                                    $rows[$rowIndex]['date'] = $thaiDateWithYear;
                                    $rows[$rowIndex]['location'] = "จ.$currentProvince-จ.$nextProvince";
                                } else {
                                    $rows[$rowIndex]['date'] = $thaiDateWithYear;
                                    $rows[$rowIndex]['location'] = "จ.$currentProvince";
                                }
                                $rows[$rowIndex]['task'] = $taskChunks[$i] ?? '';
                            } else {
                                $rows[$rowIndex]['location'] = $locationChunks[$i] ?? '';
